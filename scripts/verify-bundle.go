...
func createOrasContainer(ctx context.Context, regIP string, bundlePath string) testcontainers.Container {
	reqOras := testcontainers.ContainerRequest{
		...
		Cmd:   append([]string{"push", fmt.Sprintf("%s:5111/misscan-test:latest", regIP)}, OrasPush...),
		...
		WaitingFor: wait.ForLog("Pushed [registry] localhost:5111/misscan-test:latest"),
	}
...
func createTunnelContainer(ctx context.Context, regIP string) testcontainers.Container {
	reqTunnel := testcontainers.ContainerRequest{
		...
		Cmd:   []string{"--debug", "config", fmt.Sprintf("--policy-bundle-repository=%s:5111/misscan-test:latest", regIP), "."},
		HostConfigModifier: func(config *container.HostConfig) {
			config.NetworkMode = "host"
		},
